...
async function searchJackett(query, type) {
    try {
        ...
        return (res.data.Results || []).map(r => ({ 
            ...
            magnet: r.MagnetUri || r.Link, engine: 'Jackett'
        }));
    } catch (e) { return []; }
}
...
async function searchProwlarr(query, type) {
    try {
        ...
        return (res.data || []).map(r => ({ 
            title: r.title, seeders: r.seeders, size: r.size, 
            magnet: r.guid || r.downloadUrl, engine: 'Prowlarr'
        }));
    } catch (e) { return []; }
}
...
async function searchDirect(query) {
    try {
        ...
        return (res.data || []).filter(r => r.id !== '0').map(r => ({
            ...
            magnet: `magnet:?xt=urn:btih:${r.info_hash}&dn=${encodeURIComponent(r.name)}`, engine: 'Direct'
        }));
    } catch (e) { return []; }
}
...
function analyzeTorrent(title, meta) {
    ...
    // Less aggressive filtering to ensure results show up
...
builder.defineStreamHandler(async ({ type, id }) => {
    try {
        ...
        // RUN ALL IN PARALLEL
        const [jResults, pResults, dResults] = await Promise.all([
            searchJackett(query, type),
            searchProwlarr(query, type),
            searchDirect(query)
        ]);

        const combined = [...jResults, ...pResults, ...dResults];
        ...
        // Sort by seeders to show best first
        streams.sort((a, b) => {
            const sA = parseInt(a.title.match(/👤 (\d+)/)?.[1] || 0);
            const sB = parseInt(b.title.match(/👤 (\d+)/)?.[1] || 0);
            return sB - sA;
        });
...
app.listen(7000, () => console.log('Combined PRO v3.0.2 running...'));
