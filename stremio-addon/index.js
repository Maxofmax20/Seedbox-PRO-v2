...
const manifest = {
    id: 'org.myseedbox.sandbox.v3', version: '3.3.1', name: 'SandBox ULTIMATE Node',
    description: 'Real-Time Health Monitoring & Automated Seedbox.',
...
builder.defineStreamHandler(async ({ type, id }) => {
    try {
        ...
        const streams = [...j, ...p].map(r => ({
            name: `[ULTIMATE]`, title: `${r.title}\n👤 ${r.seeders || '?'} | 💾 ${(r.size / 1024 / 1024 / 1024).toFixed(2)} GB`,
...
setInterval(async () => {
    try {
        ...
        const health = { cpu: Math.round(cpu.currentLoad), ram: Math.round((mem.active / mem.total) * 100), disk: Math.round(fsSize[0].use) };
        const streams = Array.from(activeEngines.values()).map(e => ({ name: e.displayName || "Init...", progress: e.progress || 0, speed: e.swarm ? (e.swarm.downloadSpeed() / 1024).toFixed(1) : "0.0", peers: e.swarm ? e.swarm.wires.length : 0 }));
        let torrents = [];
        try {
        ...
        io.emit('pulse', { streams, torrents, health });
    } catch (e) {}
}, 2000);

// --- 4. ROUTES ---
app.post('/api/login', (req, res) => { if (req.body.username === USER && req.body.password === PASS) { req.session.user = USER; return res.json({ success: true }); } res.status(401).json({ error: 'Invalid' }); });
...
app.delete('/api/torrents/:hash', isAuthenticated, async (req, res) => { try { await qbtRequest('POST', '/api/v2/torrents/delete', { hashes: req.params.hash, deleteFiles: 'true' }); res.json({ success: true }); } catch (e) { res.status(500).json({ error: e.message }); } });
...
app.get('/play', async (req, res) => {
    ...
    if (magnet.startsWith('http')) { try { const head = await axios.head(magnet, { maxRedirects: 0, validateStatus: null }); if (head.headers.location) magnet = head.headers.location; } catch (e) {} }
    ...
    if (!engine) {
        ...
        engine.on('ready', () => { const f = engine.files.reduce((a, b) => a.length > b.length ? a : b); engine.mainFile = f; engine.displayName = f.name; f.select(); });
        const pInt = setInterval(() => { if (engine.mainFile && engine.bitfield) { const t = Math.ceil(engine.mainFile.length / engine.torrent.pieceLength); let dl = 0; for (let i=0; i<t; i++) if (engine.bitfield.get(i)) dl++; engine.progress = ((dl / t) * 100).toFixed(1); } }, 2000);
        engine.on('close', () => { clearInterval(pInt); activeEngines.delete(magnet); });
    }
    const serve = () => {
...
server.listen(7000, () => console.log('SandBox ULTIMATE v3.3.1 Fixed Ready.'));
